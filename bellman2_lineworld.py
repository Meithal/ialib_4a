...
for s in range(1,nb_states-2):
  p[s][1][s+1][1] = 1.0  ## on ne gagne aucun reward
for s in range(2,nb_states-1):
  p[s][0][s-1][1] = 1.0  ## on ne gagne aucun reward
p[nb_states-2][1][nb_states-1][2] = 1.0  ## on gagne si on se deplace a droite et qu'on est à l'avant derniere case
...
for state, actions in Q.items():
    print(f"Valeurs Q pour l'état {state} :", actions)
end_time = time.time()
print(f"Temps d'exécution Monte Carlo On Policy : {end_time - start_time:.4f} secondes\n")
...
print(iterative_policy_evaluation(pi_right, S, A, R, p, T))
print("Iterative policy iteration pour policy all_left : ")
pi_left = [[0.0 if a == 1 else 1.0 for a in A] for s in S]
print(iterative_policy_evaluation(pi_left, S, A, R, p, T))
end_time = time.time()
print(f"Temps d'exécution Policy Iteration : {end_time - start_time:.4f} secondes\n")

# Value Iteration
start_time = time.time()
print("\nValue Iteration")
print(Value_Iteration(S, A, R, p))
end_time = time.time()
